...
- `[x]` agent-first node discovery exists through `homebase daemon start` and `homebase connect scan`
...
- `[~]` document how `homebase daemon start` should run persistently on a node
